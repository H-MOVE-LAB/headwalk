...
def align_imu_to_gravity_with_external_file(
    acc_ss: np.ndarray,
    acc: np.ndarray,
    gyr: np.ndarray,
    sampling_rate_hz: float,
    ...
    The rotation is estimated from the mean acceleration measured during a static period retrieved from an external file,
    assuming the sensor is not moving and acceleration is dominated by gravity.

    Parameters
    ----------
    acc_ss : ndarray, shape (N, 3)
        Accelerometer data in sensor frame in external static file [m/s^2 or g].
    acc : ndarray, shape (N, 3)
        Accelerometer data in sensor frame [m/s^2 or g].
    gyr : ndarray, shape (N, 3)
        Gyroscope data in sensor frame [rad/s].
    ...
    g_measured = normalize(acc_ss[:n_static].mean(axis=0))
    g_ideal = normalize(gravity_ideal)
